web: trap '' SIGTERM; if [ "$RUN_STREAMING" != "true" ]; then BIND=0.0.0.0 bundle exec puma -C config/puma.rb & else BIND=0.0.0.0 node ./streaming & fi; if [ "$RUN_WORKER" == "true" ]; then sleep 25; if [ "$RUN_SCHEDULE" == "true" ]; then bundle exec sidekiq -c ${SIDEKIQ_THREADS:-5} & else bundle exec sidekiq -c ${SIDEKIQ_THREADS:-5} -q default -q ingress -q mailers -q pull -q push -q fasp & fi; fi; wait -n; kill -SIGTERM -$$; wait
worker: bundle exec sidekiq -c ${SIDEKIQ_THREADS:-5} -q default -q ingress -q mailers -q pull -q push 
