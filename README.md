...
## Modifications
[This repository](https://github.com/zunda/mastodon/tree/zunda-ninja-master-on-heroku) contains modifications for smoother operation on Heroku
 [zunda](https://github.com/zunda) to the [original version](https://github.com/tootsuite/mastodon). Namely:

- Apr 2017: Modified [`Procfile`](./Procfile) so that the `web` dyno can run the main app (`puma`), worker (`sidekiq`), and/or streaming API (`
m`). Set config vars `RUN_WEB`, `RUN_WORKER`, and/or `RUN_STREAMING` to `true` to run corresponding service.
